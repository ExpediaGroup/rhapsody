 ...
 */
package com.expedia.rhapsody.samples.parallelism;
...
import com.expedia.rhapsody.kafka.factory.KafkaFluxFactory;
...
/**
 * This sample shows how to process Kafka records with arbitrarily high parallelism. You can try
 * changing the number of number of samples and/or number of processing groups to see significant
 * changes in processing speed. Note that in-order offset acknowledgement is handled by
 * {@link OrderManagingReceiverAcknowledgementStrategy} such that offset commits are not executed
 * past any record whose offset we have not yet fully processed (acknowledged)
 */
public class KafkaArbitraryParallelism {
    ...
    public static void main(String[] args) throws Exception {
        ...
        kafkaSubscriberConfig.put(CommonClientConfigs.CLIENT_ID_CONFIG, KafkaArbitraryParallelism.class.getSimpleName());
        ...
        kafkaPublisherConfig.put(CommonClientConfigs.CLIENT_ID_CONFIG, KafkaArbitraryParallelism.class.getSimpleName());
        kafkaPublisherConfig.put(ConsumerConfig.GROUP_ID_CONFIG, KafkaArbitraryParallelism.class.getSimpleName());
        ...
        new KafkaValueFluxFactory<String>(kafkaPublisherConfig)
            ...
            .groupBy(acknowledgeable -> Math.abs(UUID.fromString(acknowledgeable.get()).hashCode() % NUM_GROUPS))
